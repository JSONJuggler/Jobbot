...
export const getMangaChapterPages = (chapterLandingUrl: string) => async (
  ...
  try {
    ...
    const remainingPageUrls = data?.chapterPageUrls.slice(
      4,
      data.chapterPageUrls.length,
    );

    while (remainingPageUrls?.length) {
      let nextPageUrl = remainingPageUrls.shift();

      let res: ApiResponse<ChapterPage> = await api.get(
        '/api/manga/page?pageUrl=' + nextPageUrl,
      );
